...
if (isset($_GET['fev1_id'])) {
    ...
    if (isset($_GET['confirm'])) {
        if ($_GET['confirm'] == 'yes') {
            ...
            $stmt = $pdo->prepare('DELETE FROM fev1s WHERE fev1_id = ?');
            $stmt->execute([$_GET['fev1_id']]);
            $msg = 'You have deleted the FEV1!';
        } else {
            // User clicked the "No" button, redirect them back to the read page
            header('Location: fev1s_read.php');
            exit;
        }
    }
} else {
...
<div class="content delete">
    ...
    <div class="yesno">
        <a href="fev1s_delete.php?fev1_id=<?=$fev1['fev1_id']?>&confirm=yes">Yes</a>
        <a href="fev1s_delete.php?fev1_id=<?=$fev1['fev1_id']?>&confirm=no">No</a>
    </div>
    <?php endif; ?>
</div>
